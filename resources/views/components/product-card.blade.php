...
<div class="group bg-dark-800/50 backdrop-blur-sm border border-dark-700/50 rounded-xl overflow-hidden hover:border-brand-500/50 hover:shadow-glow transition-all duration-300 flex flex-col h-full">
    <a href="{{ route('products.show', $product->id) }}" class="block flex-1 flex flex-col">
        ...
        <div class="relative aspect-square bg-dark-900 overflow-hidden">
            ...
            @if($product->discount_percentage > 0)
                <div class="absolute top-3 right-3">
                    <span class="px-3 py-1 bg-brand-600/90 backdrop-blur-sm text-white text-xs font-bold rounded-full">
        ...
        <div class="p-4 flex-1 flex flex-col">
            ...
            <!-- Product Name -->
            <h3 class="font-heading font-semibold text-white text-base mb-2 line-clamp-2 group-hover:text-brand-400 transition-colors">
                {{ $product->name }}
            </h3>
            ...
            @if($product->reviews && $product->reviews->count() > 0)
                <div class="flex items-center gap-2 mb-2">
                    <div class="flex items-center">
                        ...
                        @endphp
                        @for($i = 1; $i <= 5; $i++)
                            <svg class="w-4 h-4 {{ $i <= $rating ? 'text-yellow-400' : 'text-dark-600' }}" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                        @endfor
                    </div>
                    <span class="text-xs text-dark-400">({{ $product->reviews->count() }})</span>
                </div>
            @endif
    ...
    @if($product->stock > 0)
        <div class="p-4 pt-0">
            <button
                ...
                class="w-full px-4 py-2.5 bg-gradient-to-r from-brand-600 to-purple-600 hover:from-brand-500 hover:to-purple-500 text-white text-sm font-medium rounded-lg shadow-glow-sm hover:shadow-glow transition-all">
                Add to Cart
            </button>
        </div>
    @else
        <div class="p-4 pt-0">
            <button
                disabled
                class="w-full px-4 py-2.5 bg-dark-700 text-dark-500 text-sm font-medium rounded-lg cursor-not-allowed">
                Out of Stock
            </button>
        </div>
    @endif
